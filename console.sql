...
begin
    ...
    if sortBy = 'NAME' then
        if orderBy = 'asc' then
            select * from Students s join Account A on A.id = s.id_account
            where status != 'BLOCKED'
            order by name limit limit_in offset offset_in;
        else
            select * from Students s join Account A on A.id = s.id_account
            where status != 'BLOCKED'
            order by name desc limit limit_in offset offset_in;
        end if ;
    elseif sortBy = 'EMAIL' then
        if orderBy = 'asc' then
        select * from Students s join Account A on A.id = s.id_account
        where status != 'BLOCKED'
        order by email limit limit_in offset offset_in;
        else
            select * from Students s join Account A on A.id = s.id_account
            where status != 'BLOCKED'
            order by email desc limit limit_in offset offset_in;
        end if ;
    else
        if orderBy = 'asc' then
            select * from Students s join Account A on A.id = s.id_account
            where status != 'BLOCKED'
            order by id_account limit limit_in offset offset_in;
        else
            select * from Students s join Account A on A.id = s.id_account
            where status != 'BLOCKED'
            order by id_account desc limit limit_in offset offset_in;
        end if ;
    end if ;
end //
DELIMITER ;
